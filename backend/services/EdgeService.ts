...
// CACHE ROOT (Filesystem L2 Cache) - Persiste downloads de pacotes
...
export class EdgeService {
    ...
    private static async fetchModuleSource(specifier: string): Promise<string> {
        ...
        if (!specifier.startsWith('http://') && !specifier.startsWith('https://') && !specifier.startsWith('.')) {
            // Se não é URL nem relativo, assume pacote NPM via esm.sh (Deno-friendly)
        ...
        // Impede que o código importe 'http://169.254.169.254/metadata' ou serviços internos
    ...
    public static async execute(
        ...
        // Memória isolada mais generosa para lidar com imports
        ...
        try {
            ...
            // O Linker é a mágica: Ele resolve imports recursivamente
            await module.instantiate(scriptContext, async (specifier, referrer) => {
                // Se for um path relativo (./util.js), resolve baseado no referrer (não suportado 100% sem filesystem real,
                // mas para imports remotos funciona bem). 
                // Para simplificar, assumimos imports absolutos ou pacotes NPM.
            ...
            // Prepara o contexto de entrada (Request Body/Query)
            const reqCopy = new ivm.ExternalCopy(context).copyInto();
            
            // Executa a função default
            ...
            // Sanitização de Erro: Não vazar stack traces internos do Node
