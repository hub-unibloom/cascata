...
export class ExtensionService {
    ...
    private static async phantomInject(source: PhantomSource, requestedExtension: string): Promise<void> {
        ...
        try {
            ...
            // Resolve the actual Docker volume name by finding it via docker inspect
            // Docker Compose prefixes volume names with the project name
            let volumeName = '';
            try {
                const { stdout } = await execAsync(
                    `docker inspect cascata-db --format '{{range .Mounts}}{{if eq .Destination "/cascata_extensions"}}{{.Name}}{{end}}{{end}}'`,
                    { timeout: 10000 }
                );
                volumeName = stdout.trim();
            } catch {
                // Fallback: try common compose volume naming patterns
                try {
                    const { stdout } = await execAsync(
                        `docker volume ls --format '{{.Name}}' | grep extension_payloads | head -1`,
                        { timeout: 10000 }
                    );
                    volumeName = stdout.trim();
                } catch {
                    throw new Error('Could not resolve extension_payloads Docker volume. Is the DB container running?');
                }
            }

            if (!volumeName) {
                throw new Error('extension_payloads volume not found. Ensure docker-compose is running.');
            }

            console.log(`[ExtensionService] Resolved volume: ${volumeName}`);

            // The magic command: extract extension files from the official image
            // into our shared volume. The --rm flag ensures zero leftover containers.
            //
