...
export class BackupService {
    ...
    /**
     * Gera um arquivo de backup em disco temporário e retorna o caminho.
     * Evita OOM ao não manter o buffer inteiro na RAM.
     */
    public static async generateBackupToFile(project: ProjectMetadata): Promise<string> {
        ...
        try {
            ...
            // 4. SCHEMA
            // Pipe direto do stdout do pg_dump para o archiver
            ...
            archive.abort();
            output.destroy(); // Fecha o stream
            if (fs.existsSync(tempFilePath)) fs.unlinkSync(tempFilePath); // Limpa lixo
            throw e;
        }
    }
