...
export class StorageController {
    ...
    static async createBucket(req: CascataRequest, res: any, next: any) {
        try {
            const p = StorageController.getSafePath(STORAGE_ROOT, req.project.slug, req.body.name);
            await fs.mkdir(p, { recursive: true });
            res.json({ success: true });
        } catch (e: any) {
            if (e.status === 403) return res.status(403).json({ error: 'Access Denied: Invalid bucket name' });
            next(e);
        }
    }
    ...
    static async renameBucket(req: CascataRequest, res: any, next: any) {
        ...
        try {
            const projRoot = StorageController.getSafePath(STORAGE_ROOT, req.project.slug);
    ...
    static async createFolder(req: CascataRequest, res: any, next: any) {
        try {
            ...
            const bucketPath = StorageController.getSafePath(STORAGE_ROOT, req.project.slug, req.params.bucket);
            ...
            if (e.status === 403) return res.status(403).json({ error: 'Access Denied: Path Traversal' });
            res.status(500).json({ error: e.message });
        }
    }
    ...
    static async uploadFile(req: CascataRequest, res: any, next: any) {
        ...
        try {
            ...
            if (storageConfig.provider === 'local') {
                const dest = StorageController.getSafePath(STORAGE_ROOT, req.project.slug, bucket, relativePath, req.file.originalname);
    ...
    static async moveFiles(req: CascataRequest, res: any, next: any) {
        ...
        try {
            ...
            const root = StorageController.getSafePath(STORAGE_ROOT, req.project.slug);
    ...
    static async deleteObject(req: CascataRequest, res: any, next: any) {
        ...
        try {
            if (storageConfig.provider === 'local') {
                const bucketRoot = StorageController.getSafePath(STORAGE_ROOT, req.project.slug, req.params.bucket);
