...
import { 
  ...
  Search, Server, FileJson, CheckSquare, Square, AlertTriangle, Zap
} from 'lucide-react';
...
const ProjectIntelligence: React.FC<{ projectId: string }> = ({ projectId }) => {
  ...
  const [aiGovernance, setAiGovernance] = useState({ 
      mcp_enabled: true, 
      mode: 'read_write' as 'read_only' | 'read_write', 
    ...
    try {
        ...
        if (current) {
            ...
            if (current.metadata?.ai_governance) {
                setAiGovernance(current.metadata.ai_governance);
            }
        }
        fetchTables();
    } catch (e) {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8 pb-40">
        ...
        <div className="flex items-end justify-between">
            <div>
                 <h1 className="text-4xl font-black text-slate-900 tracking-tighter mb-2">Agent Intelligence</h1>
            ...
            <div className="bg-indigo-50 text-indigo-700 px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 border border-indigo-100">
                <Brain size={16}/> MCP Protocol v1.0 Supported
            </div>
        </div>
                ...
                <div className="bg-gradient-to-br from-slate-50 to-white border border-slate-200 rounded-[3rem] p-8 shadow-sm">
                    ...
                    <ul className="space-y-3">
                        <li className="flex items-start gap-3 text-xs text-slate-600">
                            ...
                            <span>Agentes usam a <strong>Service Role</strong> para introspecção do schema.</span>
                        </li>
                        <li className="flex items-start gap-3 text-xs text-slate-600">
                            <CheckCircle2 size={16} className="text-emerald-500 shrink-0 mt-0.5"/>
                            <span>Logs de auditoria registram todas as queries geradas por IA.</span>
                        </li>
                        <li className="flex items-start gap-3 text-xs text-amber-600 bg-amber-50 p-2 rounded-lg border border-amber-100">
                            <AlertTriangle size={16} className="shrink-0 mt-0.5"/>
                            <span>No modo <strong>Full Power</strong>, o agente pode deletar dados. Use com cuidado.</span>
                        </li>
                    </ul>
                </div>
            </div>
            ...
            <div className="xl:col-span-2 space-y-8">
                <div className="bg-white border border-slate-200 rounded-[4rem] p-12 shadow-sm relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                        {/* Toggle Master */}
                        <div className={`rounded-[2.5rem] p-8 border transition-all ${aiGovernance.mcp_enabled ? 'bg-emerald-50 border-emerald-100' : 'bg-slate-50 border-slate-200'}`}>
                        ...
                        <div className="bg-slate-50 rounded-[2.5rem] p-8 border border-slate-200">
                             <div className="flex justify-between items-center mb-4">
                                <span className="text-sm font-black text-slate-900 uppercase tracking-widest">Permission Mode</span>
                                <div className="flex bg-white rounded-xl p-1 border border-slate-200 shadow-sm">
                                    <button 
                                        onClick={() => setAiGovernance(prev => ({ ...prev, mode: 'read_only' }))}
                                        className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${aiGovernance.mode === 'read_only' ? 'bg-emerald-100 text-emerald-700 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                                    >
                                        Read-Only
                                    </button>
                                    <button 
                                        onClick={() => setAiGovernance(prev => ({ ...prev, mode: 'read_write' }))}
                                        className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${aiGovernance.mode === 'read_write' ? 'bg-rose-100 text-rose-700 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                                    >
                                        Full Power
                                    </button>
                                </div>
                            ...
                            <p className="text-[10px] text-slate-500 font-medium leading-relaxed">
                                {aiGovernance.mode === 'read_only' 
                                    ? "Modo Seguro. Agentes podem apenas LER dados e schemas. CREATE/UPDATE/DELETE bloqueados." 
                                    : "Modo Irrestrito. Agentes podem criar tabelas, inserir dados e modificar estrutura."}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <label className="text-sm font-black text-slate-900 uppercase tracking-widest block">Context Scope</label>
                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Quais tabelas a IA pode "ver"?</p>
                            </div>
                            <div className="flex gap-2">
                                ...
                                <button onClick={() => setAiGovernance(prev => ({ ...prev, allowed_tables: availableTables }))} className="text-[10px] font-bold text-indigo-600 bg-indigo-50 px-3 py-2 rounded-xl hover:bg-indigo-100 transition-colors">Select All</button>
                                <button onClick={() => setAiGovernance(prev => ({ ...prev, allowed_tables: [] }))} className="text-[10px] font-bold text-slate-500 bg-slate-100 px-3 py-2 rounded-xl hover:bg-slate-200 transition-colors">Clear</button>
                            </div>
                        </div>
                        
                        <div className="bg-slate-50 rounded-[2rem] p-2 border border-slate-200 min-h-[300px]">
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 max-h-[400px] overflow-y-auto custom-scrollbar p-2">
                                {filteredTables.map(t => {
                                    const isAllowed = aiGovernance.allowed_tables.includes(t);
                                    return (
                                        <button 
                                            key={t}
                                            onClick={() => {
                                                const newSet = new Set(aiGovernance.allowed_tables);
                                                if (newSet.has(t)) newSet.delete(t); else newSet.add(t);
                                                setAiGovernance(prev => ({ ...prev, allowed_tables: Array.from(newSet) }));
                                            }}
                                            className={`flex items-center gap-3 px-4 py-3 rounded-xl border transition-all text-left group ${isAllowed ? 'bg-white border-indigo-200 shadow-sm ring-1 ring-indigo-500/20' : 'border-transparent hover:bg-white hover:shadow-sm'}`}
                                        >
                                            <div className={`w-5 h-5 rounded flex items-center justify-center transition-colors ${isAllowed ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-400 group-hover:bg-slate-300'}`}>
                                                {isAllowed ? <CheckSquare size={12}/> : <Square size={12}/>}
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <span className={`text-xs font-bold truncate block ${isAllowed ? 'text-indigo-900' : 'text-slate-500'}`}>{t}</span>
                                            </div>
                                            {isAllowed ? <ScanEye size={14} className="text-indigo-400"/> : <EyeOff size={14} className="text-slate-300"/>}
                                        </button>
                                    )
                                })}
                                {filteredTables.length === 0 && <p className="col-span-full text-center py-10 text-xs text-slate-400 italic">Nenhuma tabela encontrada.</p>}
                            </div>
                        </div>
                    </div>
                </div>
