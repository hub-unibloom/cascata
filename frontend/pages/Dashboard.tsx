...
const Dashboard: React.FC<DashboardProps> = ({ onSelectProject }) => {
  ...
  // Strategies
  const [importMode, setImportMode] = useState<'recovery' | 'template'>('recovery');
  ...
  const handleUploadAnalyze = async () => {
      ...
      try {
          ...
          setImportSlug(baseSlug + '-imported');
      ...
      try {
          const res = await fetch('/api/control/projects/import/confirm', {
              ...
              body: JSON.stringify({
                  ...
                  slug: importSlug,
                  name: importName, // Send the overridden name
                  mode: importMode,
          ...
          setImportError(e.message);
          setImportStep('config'); // Go back to config on error
      }
  };
              ...
              <div className="flex items-center justify-between mb-8 shrink-0">
                  <div className="flex items-center gap-6">
                      ...
                      <div>
                          ...
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Cascata Archive Format (.caf)</p>
                      </div>
                  </div>
              ...
              <div className="flex-1 overflow-y-auto pr-2">
                  ...
                  {importStep === 'config' && importManifest && (
                      <div className="space-y-8 animate-in slide-in-from-right-4">
                          ...
                          <div className="space-y-4">
                              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Import Strategy</label>
                              <div className="grid grid-cols-2 gap-4">
                                  <button 
                                    onClick={() => { setImportMode('recovery'); setImportSlug(importManifest.project.slug); setImportName(importManifest.project.name); }}
                                    className={`p-6 rounded-[2rem] border text-left transition-all relative overflow-hidden ${importMode === 'recovery' ? 'bg-indigo-600 border-indigo-600 text-white shadow-xl' : 'bg-white border-slate-200 hover:border-indigo-300'}`}
                                  >
                                      <RefreshCcw size={24} className="mb-4"/>
                                      <span className="block font-black text-sm uppercase tracking-tight">Recovery</span>
                                      <span className={`text-[10px] font-medium mt-1 block leading-tight ${importMode === 'recovery' ? 'text-indigo-200' : 'text-slate-400'}`}>Restore original keys & users. Exact clone.</span>
                                  </button>

                                  <button 
                                    onClick={() => { setImportMode('template'); setImportSlug(importManifest.project.slug + '-copy'); setImportName(importManifest.project.name + ' (Copy)'); }}
                                    className={`p-6 rounded-[2rem] border text-left transition-all relative overflow-hidden ${importMode === 'template' ? 'bg-indigo-600 border-indigo-600 text-white shadow-xl' : 'bg-white border-slate-200 hover:border-indigo-300'}`}
                                  >
                                      <Copy size={24} className="mb-4"/>
                                      <span className="block font-black text-sm uppercase tracking-tight">Template</span>
                                      <span className={`text-[10px] font-medium mt-1 block leading-tight ${importMode === 'template' ? 'text-indigo-200' : 'text-slate-400'}`}>Generate new keys. Clean state. Ideal for forking.</span>
                                  </button>
                              </div>
                          </div>
                          ...
                          <div className="space-y-6">
                              ...
                              {importMode === 'template' && (
                                  <div 
                                    ...
                                    className="flex items-center gap-4 p-4 bg-slate-50 rounded-[1.5rem] border border-slate-200 cursor-pointer"
                                  >
                                      ...
                                      <div>
                                          <span className="text-xs font-bold text-slate-700 block">Include Public Data</span>
                                          <span className="text-[10px] text-slate-400">Import CSV rows from public schema tables.</span>
                                      </div>
                                  </div>
                              )}
                          </div>
                          ...
                          <div className="flex gap-4 pt-4">
                              ...
                              <button onClick={handleExecuteImport} className="flex-[2] py-5 bg-emerald-600 text-white font-black rounded-[2rem] shadow-xl hover:bg-emerald-700 transition-all uppercase tracking-widest text-xs flex items-center justify-center gap-2">
                                  <CheckCircle2 size={16}/> Execute Import
                              </button>
                          </div>
                      </div>
                  )}
                  ...
                  {importStep === 'processing' && (
                      <div className="py-20 flex flex-col items-center justify-center text-center animate-in zoom-in-95">
                          ...
                          <h3 className="text-2xl font-black text-slate-900 mb-2 tracking-tight">Restoring Infrastructure...</h3>
                          <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px] max-w-xs mx-auto leading-relaxed">
                              Injecting SQL Schema, Hydrating Storage & Indexing Vectors.
                          </p>
                      </div>
                  )}
              </div>
