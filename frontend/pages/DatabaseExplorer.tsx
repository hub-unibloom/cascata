...
import { 
  ...
  MessageSquare, RefreshCcw, FileType, Shield, Eye, AlertTriangle
} from 'lucide-react';
...
const DatabaseExplorer: React.FC<{ projectId: string }> = ({ projectId }) => {
  ...
  const handleCreateTableSubmit = async () => {
    ...
    try {
      ...
      // 1. Create Structure via REST
      await fetchWithAuth(`/api/data/${projectId}/tables`, {
        method: 'POST',
        body: JSON.stringify(payload)
      });

      // 2. Import Data (Batch Process via REST)
      if (importPendingData) {
          const chunkSize = 100; // Safe limit for REST
                  ...
                  newTableCols.forEach(col => {
                      if (col.sourceHeader && row.hasOwnProperty(col.sourceHeader)) {
                          ...
                          if (val === '' || val === undefined) {
                              // If it's nullable, send NULL. Else let DB default handle it (undefined)
                              if (col.isNullable) val = null;
                              else if (col.defaultValue) val = undefined;
                          }
                          newRow[col.name] = val;
                      }
                  });
      ...
      if (importPendingData || newTableName) {
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <div className={`fixed inset-y-0 right-0 w-[500px] bg-white shadow-2xl z-[100] transform transition-transform duration-300 ease-in-out flex flex-col border-l border-slate-200 ${showCreateTable ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="p-6 border-t border-slate-100 bg-slate-50/50 flex gap-4">
             ...
             <button onClick={handleCreateTableSubmit} disabled={executing} className="flex-[2] bg-indigo-600 text-white py-3 rounded-xl text-xs font-black uppercase tracking-widest shadow-xl hover:bg-indigo-700 transition-all flex items-center justify-center gap-2">{executing ? <Loader2 size={14} className="animate-spin"/> : importPendingData ? 'Deploy & Import' : 'Deploy Table'}</button>
          </div>
        </div>
      </div>
